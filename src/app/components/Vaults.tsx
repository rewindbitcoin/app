...
const Vaults = ({ vaults }: { vaults: VaultsType }) => {
  ...
  console.log(handleDelegateVaultMap);
  ...
  return (
    <View className="gap-4">
      {Object.values(vaults).map(vault => (
        <View
          key={vault.vaultId}
          className="max-w-[90%] items-center rounded-3xl bg-white p-2"
        >
          <Text>{vault.vaultId}</Text>
          <Button
            mode="text"
            onPress={() => {
              //TODO: when fixed the problem then simply use onPress={handleDelegateVaultMap[vault.vaultId]}
              console.log(vault.vaultId);
              const f = handleDelegateVaultMap[vault.vaultId];
              if (!f) throw new Error('does not exist');
              f();
            }}
          >
